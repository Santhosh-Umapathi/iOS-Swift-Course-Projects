...
class AppDelegate: UIResponder, UIApplicationDelegate
{
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplication.LaunchOptionsKey: Any]?) -> Bool {
        ...
        let path = Bundle.main.path(forResource: "DataModel", ofType: "realm")
        if !FileManager.default.fileExists(atPath: defaultPath!), let bundledPath = path
        {
            do
            {
                ...
                print("Success copying Pre-Populated DataBase Model Realm")
            }
            catch { print("Error copying Pre-Populated DataBase Model Realm \(error)") }
        }
